		...
		<DialogPrimitive.Root open={open} onOpenChange={onOpenChange}>
			<DialogPrimitive.Portal>
				...
				<DialogPrimitive.Content
					className={cn(
						"firm-paper fixed inset-x-0 bottom-0 z-50 max-h-[92vh] overflow-y-auto rounded-none border-t border-[var(--pon-ink)] p-4",
				...
				>
					...
					<div className="mx-auto mb-3 h-[3px] w-10 bg-[var(--pon-line-2)]" aria-hidden />

					<div className="mb-3 flex items-center justify-between">
						<DialogPrimitive.Title className="t-h3 text-[var(--pon-fg-0)]">
							{title}
						</DialogPrimitive.Title>
						<DialogPrimitive.Close className="inline-flex size-7 items-center justify-center rounded-[var(--pon-r-sm)] border border-[var(--pon-line-2)] text-[var(--pon-fg-3)] transition-colors hover:border-[var(--pon-fg-3)] hover:text-[var(--pon-fg)]">
...
export function MobileActionBar({ children }: { children: ReactNode }) {
	return (
		<div
			className="fixed inset-x-0 z-40 border-t border-[var(--pon-line-2)] bg-[var(--pon-bg)] px-3 py-3 lg:hidden"
