...
async function fetchTokenDecimalsFromBSCScan(tokenAddress) {
	try {
		...
		if (data.status === "1" && data.result) {
			return {
				decimals: parseInt(data.result[0]?.divisor || data.result[0]?.decimals || "18", 10),
...
async function fetchTokenDecimals(tokenAddress, network = "bsc") {
	...
	const tokenData = await fetchFromGeckoTerminal(tokenAddress, network);
	...
	if (network === "bsc") {
		const bscData = await fetchTokenDecimalsFromBSCScan(tokenAddress);
...
function saveResults(results) {
	...
	const csvHeaders =
		"Pair Address,Base Token Address,Base Token Symbol,Base Token Name,Base Decimals,Base Decimal Source,Quote Token Address,Quote Token Symbol,Quote Token Name,Quote Decimals,Quote Decimal Source,Chain ID,Dex ID\n";
