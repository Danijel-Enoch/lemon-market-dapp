...
export function createOpenRouterAdvisor(options: AdvisorOptions): Advisor {
	...
	return async (request: AdviceRequest): Promise<Advice> => {
		...
		try {
			...
			const parsed = JSON.parse(content) as { action?: string; rationale?: string };
			...
			if (!request.options.some((o) => o.kind === action)) {
				throw new Error(
					`Model chose "${action}", which is not among ${request.options.map((o) => o.kind).join(", ")}`,
				);
			}
...
The vault holds a long spot position and an equal-notional short perpetual against it, and earns funding. A separate policy engine has already decided which actions are safe and sized them. Your only job is to choose which of the offered actions to take right now.

Rules:
- Choose exactly one action, from the list given. Never invent one.
- You cannot change any amount. Sizes are fixed by the policy.
...
function renderSituation(request: AdviceRequest): string {
	...
	const annualFunding = (s.fundingShortPercentPerHour * 24 * 365).toFixed(2);
	...
	return [
		...
		"",
		"Permitted actions:",
		...options.map((o) => `- ${o.kind}${o.amount > 0n ? ` (${usd(o.amount)})` : ""}: ${o.reason}`),
	].join("\n");
}
