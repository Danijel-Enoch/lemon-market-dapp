...
| **Deposits** | Fund from any supported chain with a Relay deposit address — a plain transfer or an exchange withdrawal, no bridging UI. |
...
Perps, spot and baskets work with no configuration. Two features are gated:
...
- **Deposits** need a free `RELAY_API_KEY` from
  [dashboard.relay.link](https://dashboard.relay.link).

Both report themselves as unavailable in the UI rather than failing quietly.
...
| Relay | Cross-chain deposits | [docs.relay.link](https://docs.relay.link) |
