...
export default function LiquidityPage() {
	...
	const handleDeposit = async () => {
		...
		try {
			...
			const response = await fetch("http://localhost:3000/liquidity/vault/deposit-insurance", {
				method: "POST",
				headers: {
					Accept: "*/*",
	...
	const handleWithdraw = async () => {
		...
		setApiError(null);
		try {
			const response = await fetch("http://localhost:3000/liquidity/vault/withdraw-insurance", {
				method: "POST",
				headers: {
					Accept: "*/*",
					"Content-Type": "application/json",
