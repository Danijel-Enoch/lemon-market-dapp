    ...
    try {
      ...
      if (result.simulationResult && !result.simulationResult.success) {
        if (result.simulationResult.revertReason) {
          ...
          errorMessage =
            "Transaction would fail - please check your parameters";
        }
      }
...
export default function PerpContent() {
  ...
  const [marginTokenPriceUsd, _setMarginTokenPriceUsd] = useState<
    number | null
  >(null);
  ...
  const transactionError =
    fetcher.data?.intent === "open-position" ? fetcher.data.error : null;
                ...
                {tradingPair.assetType === "crypto"
                  ? `$${
                      marketData?.volume24h
                        ? Number(marketData.volume24h).toLocaleString()
                        : "0"
                    }`
                  : "N/A"}
              </div>
                ...
                {tradingPair.assetType === "crypto"
                  ? `$${
                      marketData?.liquidity
                        ? Number(marketData.liquidity).toLocaleString()
                        : "0"
                    }`
                  : "N/A"}
              </div>
              ...
              <div className="relative">
                ...
                <div className="absolute bottom-2 left-2 z-10 flex items-center gap-2">
                  <Tabs
                    value={chartType}
                    onValueChange={(v) =>
                      setChartType(v as "dexscreener" | "beta")
                    }
                  >
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-foreground font-medium">
                        {marginBalance
                          ? `${parseFloat(
                              formatUnits(
                                BigInt(marginBalance as string),
                                decimals
                              )
                            ).toFixed(Math.min(6, decimals))} ${"USDC"} ${
                              marginTokenPriceUsd
                                ? `(~$${marginTokenPriceUsd.toFixed(4)})`
                                : ""
                            }`
                          : `0.00 ${"USDC"}`}
                      </span>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex items-center gap-3">
                        <span className="text-xs text-muted-foreground">{`Available: ${parseFloat(
                          formatUnits(
                            BigInt((marginBalance as string) || "0"),
                            decimals
                          )
                        ).toFixed(Math.min(6, decimals))} ${"USDC"} ${
                          marginTokenPriceUsd
                            ? `(~$${availableMarginUsd.toFixed(2)})`
                            : ""
                        }`}</span>
                      </div>
                    </div>
                  ...
                  {isApprovalConfirmed && approvalHash && !needsApproval && (
                    <div className="p-3 bg-green-900/50 border border-success rounded-lg">
                      <p className="text-success text-sm">
                        ✅ {"USDC"} approval confirmed! You can now create
                        positions.
                        <a
                  ...
                  <div className="w-full mt-4">
                    <div className="flex items-center gap-3 mb-4">
                      <input
                        ...
                        onChange={(e) =>
                          setAutoSwapAndApprove(e.target.checked)
                        }
          ...
          {positions.length > 0 && (
            <div className="flex gap-4 text-sm">
              <span className="text-muted-foreground">
                Open:{" "}
                <span className="text-foreground">{openPositions.length}</span>
              </span>
              <span className="text-muted-foreground">
                Total Margin:{" "}
                <span className="text-foreground">
