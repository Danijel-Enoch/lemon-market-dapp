...
export async function GET(request: Request) {
	try {
		...
		const _chainParam = url.searchParams.get("chain") || "base";
		...
		const usdPrice =
			priceData.data?.bestPriceUSD?.priceUSD ||
			priceData.data?.bestPriceUSD?.price ||
			priceData.data?.averagePrice ||
			null;

		return NextResponse.json(
			{ success: true, priceUsd: usdPrice, raw: priceData },
			{ status: 200 },
		);
	} catch (error) {
		return NextResponse.json(
