        ...
        <div className="relative sm:max-h-[600px] md:max-h-[640px] overflow-hidden">
          <div className="grid sm:grid-cols-2 gap-6">
            <div className="flex flex-col gap-6 sm:mt-10">
              {testimonials.filter((_, idx) => idx % 2 === 0).map((t, i) => (
                <div key={`left-${i}`} className={`rounded-2xl bg-lime-800/20 p-6 border border-lime-400/60`}>
            ...
            <div className="flex flex-col gap-6">
              {testimonials.filter((_, idx) => idx % 2 !== 0).map((t, i) => (
                <div key={`right-${i}`} className={`rounded-2xl bg-lime-800/20 p-6 border border-lime-400/60`}>
                  <p className="text-white/80">“{t.quote}”</p>
                  <div className="mt-4 flex items-center gap-3">
                    <Image src={t.avatar} alt={t.author} width={40} height={40} className="rounded-full" />
                    <div>
