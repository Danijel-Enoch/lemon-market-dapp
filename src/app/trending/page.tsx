	...
	useEffect(() => {
		const fetchTrendingData = async () => {
			try {
				...
				const transformedStocks: Token[] = stocksData.data.map((stock, index) => ({
					id: index + 1,
					symbol: stock.Ticker,
					name:
						stock.Ticker === "NFLX"
					...
					marketCap: "N/A",
					trend: "up",
					logo: stock.Ticker === "NFLX" ? "🎬" : stock.Ticker === "TSLA" ? "🚗" : "📈",
				}));
					...
					<div className="space-y-8">
						<Card className="overflow-hidden border-gray-100/10">
							<CardHeader className="border-b border-gray-100/10">
								<CardTitle className="text-lg font-semibold">Top Trending Tokens</CardTitle>
							</CardHeader>
							<CardContent className="p-0">
													...
													<td colSpan={9} className="p-12 text-center">
														<div className="flex flex-col items-center gap-2">
															...
															<div className="text-sm text-muted-foreground">
																{searchQuery ? "Try adjusting your search" : "Unable to fetch data"}
															</div>
														</div>
													</td>
												</tr>
								...
								{!searchQuery && (
									<div className="p-4 border-t border-gray-100/10">
										...
										{!isLoadingMore && hasMore && <div ref={observerTarget} className="h-4" />}
